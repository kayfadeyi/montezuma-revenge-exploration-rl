...
def evaluate(model_path, environment, n_episodes=10, render=True):
    # Initialize device
    ...
    evaluator = MontezumaEvaluator(agent, environment, n_episodes=n_episodes)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate a trained model on Atari Environment')
    parser.add_argument('--env', type=str, required=True,
                        help='The atari environment to use')
    ...
    evaluate(args.model_path, args.env, args.episodes, args.render)
