...
class TrainModel:
    ...
    def train(self,
        ...
        curiosity = CuriosityModule(
            ...
            device=device,
            learning_rate=learning_rate * 5  # Decreased multiplier
        )
        reward_combiner = RewardCombiner(beta=0.8)

        memory = PrioritizedReplayBuffer(50000)  # From 5000
        ...
        for episode in tqdm(range(start_episode, num_episodes), desc="Training"):
            ...
            while not (done or truncated):
                ...
                if total_steps % 500 == 0 and device.type == 'cuda':
                    ...
                    logging.info(f'device: {test_device}')
                    logging.info(torch.cuda.memory_summary(device=test_device, abbreviated=True))
